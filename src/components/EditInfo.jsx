...
function EditInfo() {
  ...
  const { file: avatarFile, previewUrl, handleFileChange } = useImagePreview(avatarUrl || "/profileImg.png");
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col select-none">
      ...
      <main className="max-w-xl w-full m-auto py-8 px-4 flex flex-col gap-y-6">
        <div className="flex items-center gap-x-3 relative">
          <BackBtn />
          <h1 className="text-2xl font-black text-white mt-5">Edit Profile</h1>
        </div>

        <div className="bg-spotify-surface border border-spotify-card rounded-2xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-y-6 relative">
            <div className="flex flex-col items-center gap-y-2">
              <div className="relative group select-none">
                <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full overflow-hidden border-2 border-spotify-card group-hover:border-spotify-green transition-colors shadow-2xl bg-black">
                  <img
                    loading="lazy"
                  ...
                  <label
                    ...
                    className="absolute inset-0 flex flex-col items-center justify-center gap-1 opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity text-white text-xs font-bold bg-black/50"
                  >
                ...
                <label
                  htmlFor="avatar-file-input"
                  className="sm:hidden absolute bottom-0 right-0 bg-spotify-green text-black p-2 rounded-full shadow-lg cursor-pointer border-2 border-black active:scale-95 transition-all duration-200 flex items-center justify-center"
                >
            ...
            <ActionBtn
              ...
              className="w-full mt-2 bg-spotify-green text-black font-bold py-3.5 rounded-full hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            />
          </form>
        </div>
      </main>
