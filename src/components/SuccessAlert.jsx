    ...
    >
      <div className={`flex justify-between gap-3 p-3.5 sm:p-4 rounded-2xl bg-[#181818]/95 backdrop-blur-md border border-[#282828] shadow-2xl ${
        ...
        <div className={`flex gap-3 min-w-0 flex-1 ${
          ...
          {/* آیکون */}
          <div className={`p-2 rounded-xl border shrink-0 flex items-center justify-center ${ICON_STYLES[icon] || ICON_STYLES.success}`}>
            {ALERT_ICONS[icon] || ALERT_ICONS.error}
          </div>

          {/* متن‌ها */}
          <div className={`flex flex-col min-w-0 flex-1 ${hasDescription ? 'pt-0.5' : ''}`}>
            {message && (
        ...
        <div className={`flex items-center gap-1 shrink-0 ${hasDescription ? 'self-start mt-0.5' : ''}`}>
          {type === "link" && address && (
            <Link
              to={address}
              onClick={onClose}
              className="p-1.5 text-zinc-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors flex items-center justify-center cursor-pointer"
              title="View details"
            >
