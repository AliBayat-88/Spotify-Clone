...
function MobilePlayer() {
  const { isExpanded, setIsExpanded, isPlaying, togglePlay , currentSong } = usePlayer();
        ...
        <div className="w-full flex flex-col gap-y-6 mb-6">
          <div className="flex justify-between items-center w-full px-1">
            <div className="flex flex-col gap-y-1">
              ...
              <p className="text-sm font-medium text-[#b3b3b3] hover:text-white transition-colors cursor-pointer">{artist?.name}</p>
            </div>
          ...
          <div className="flex items-center justify-center gap-x-8 mt-2">
            <button className="bg-transparent border-none p-2 text-[#b3b3b3] hover:text-white active:scale-90 transition-all focus:outline-none">
              <BackWardBtn className="fill-current w-8 h-8" />
            </button>
            <div
              onClick={togglePlay}
              className="w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-md hover:scale-105 active:scale-95 transition-all cursor-pointer"
            >
              {whichIcon}
            </div>
            <button className="bg-transparent border-none p-2 text-[#b3b3b3] hover:text-white active:scale-90 transition-all focus:outline-none">
              <ForwardBtn className="fill-current w-8 h-8" />
            </button>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-white/10 shadow-lg">
                  <img src={currentSong?.cover_url} alt={artist?.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-[#a0a0a0] font-bold">About the Artist</span>
                  <h3 className="text-white font-bold text-base leading-tight mt-0.5">{artist?.name}</h3>
                </div>
              </div>
