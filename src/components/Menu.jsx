...
function Menu({
  ...
  const baseBtnClass = "group relative flex items-center gap-3 w-full bg-transparent border-none py-2.5 px-2.5 text-sm font-semibold rounded-md cursor-pointer transition-all duration-200 focus:outline-none active:scale-[0.99]";
  const defaultBtnClass = `${baseBtnClass} text-spotify-subtext hover:text-white hover:bg-[#3e3e3e]/60`;
  const dangerBtnClass = `${baseBtnClass} text-red-400 hover:text-red-300 hover:bg-red-500/10`;
  ...
  return (
    <div
      ...
      `}
    >
      {type === "public_playlist_page" && onAddToLibrary && (
        <button
          onClick={(e) => {
            if (!user) {
          ...
          className={isSavedInLibrary ? dangerBtnClass : defaultBtnClass}
        >
          {isSavedInLibrary ? (
            <>
      ...
      {type !== "artist" && type !== "playlist_page" && type !== "public_playlist_page" && (
        <>
          <button
            ...
            className={`group relative flex items-center justify-between gap-3 w-full bg-transparent border-none py-2.5 px-3 text-spotify-subtext hover:text-white text-sm font-semibold rounded-md cursor-pointer transition-all duration-200 focus:outline-none ${
              isSubOpen ? 'bg-[#3e3e3e] text-white' : 'hover:bg-[#3e3e3e]/60'
            }`}
          >
            <div className="flex gap-x-3 items-center">
              <div className="border-neutral-500 group-hover:border-white transition-colors duration-200 border-[1.5px] inline-flex p-0.5 rounded-full">
          ...
          >
            ...
            <span className={isLiked ? "text-spotify-green" : ""}>
      ...
      {type === "artist" && (
        <button
          ...
          className={
            isArtistsFollowed
              ? dangerBtnClass
              : `${baseBtnClass} text-spotify-green hover:text-spotify-green-hover hover:bg-[#3e3e3e]/60`
          }
        >
          {isArtistsFollowed ? (
          ...
          {isArtistsFollowed ? 'Unfollow' : 'Follow Artist'}
        </button>
      )}
      ...
      >
        <svg viewBox="0 0 32 32" width="20" height="20" stroke="currentColor" fill="currentColor" className="text-neutral-400 group-hover:text-white transition-colors">
          <path d="M19.2 8.8a4 4 0 1 1 0.8 2.5L12.7 15a4 4 0 0 1 0 2l7.4 3.7a4 4 0 1 1-0.8 1.5l-7.4-3.7a4 4 0 1 1 0-5l7.4-3.7A4 4 0 0 1 19.2 8.8ZM23.2 6.4a2.4 2.4 0 1 0 0 4.8 2.4 2.4 0 0 0 0-4.8Zm0 14.4a2.4 2.4 0 1 0 0 4.8 2.4 2.4 0 0 0 0-4.8ZM6.4 16a2.4 2.4 0 1 0 4.8 0 2.4 2.4 0 0 0 0-4.8Z"/>
        </svg>
        Share
      </button>
    </div>
