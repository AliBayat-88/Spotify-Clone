    ...
    <div className={`flex flex-col gap-y-2 w-full relative select-none ${disabled ? 'opacity-40 cursor-not-allowed' : ''}`} ref={dropdownRef}>
      ...
      <button
        ...
        className={`w-full bg-black text-left px-4 py-3.5 rounded-xl border transition-all duration-200 flex items-center justify-between text-sm font-medium ${
          disabled
            ? "border-spotify-card cursor-not-allowed text-gray-600"
            : isOpen
              ? "border-white ring-1 ring-white cursor-pointer"
              : "border-spotify-card hover:border-[#3e3e3e] cursor-pointer"
        }`}
      >
        <div className="flex items-center gap-2.5 truncate">
          {selectedOption && getOptionImage(selectedOption) && (
            <img
              ...
              alt=""
              className="w-5 h-5 rounded-full object-cover bg-spotify-card shrink-0"
            />
          )}
      ...
      {isOpen && !disabled && (
        <div className="absolute top-full left-0 right-0 mt-2 z-50 bg-spotify-surface border border-spotify-card rounded-xl shadow-2xl overflow-hidden py-1.5 max-h-60 overflow-y-auto animate-[fadeIn_.15s_ease-out]">
          {options.length === 0 ? (
                ...
                >
                  ...
                  {isSelected && (
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      className="w-4 h-4 text-spotify-green shrink-0 ml-2"
                    >
                      <path
                        fillRule="evenodd"
